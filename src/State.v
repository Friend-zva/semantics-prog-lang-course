...
Section S.
  ...
  *)
  ...
  - intros. inversion H. subst.
    ...
    + inversion H6; subst.
      * contradiction.
      * apply st_binds_tl. assumption. apply H13.
  - intros. inversion H; subst.
    + apply st_binds_hd.
    + apply st_binds_tl.
      * assumption.
  ...
  - subst. assert (n1 = m).
    + apply (state_deterministic st x2 n1 m SN SM).
  ...
  - inversion H5; subst.
    ...
    + apply st_binds_tl.
      * assumption.
      * apply st_binds_tl. assumption. apply H7.
  Qed.
  ...
  Lemma state_extensional_equivalence_false (a : A) :
    exists st st', (forall x z, st / x => z <-> st' / x => z) /\ (st = st' -> False).
  Proof. exists [(Id 1, a)], [(Id 1, a); (Id 1, a)]. split.
  - intros. split.
  ...
  Proof. intro state_equivalence. reflexivity. Qed.

  Lemma st_equiv_symm (st st': state) (H: st ~~ st') : st' ~~ st.
  Proof. intro state_equivalence. symmetry. apply H. Qed.

  Lemma st_equiv_trans (st st' st'': state) (H1: st ~~ st') (H2: st' ~~ st'') : st ~~ st''.
  Proof. intro state_equivalence. split.
  - intro H. apply H2. apply H1. assumption.
  - intro H. apply H1. apply H2. assumption.
  Qed.
