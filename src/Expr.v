...
Module SmokeTest.
  ...
  Proof. exists (Id 1), [].
    intro H. inversion H. subst. inversion VALA. subst. inversion VAR.
  Qed.
  ...
  Proof. inversion HH. subst. inversion VALB. subst.
    replace (za * 2)%Z with (za + za)%Z by lia. apply bs_Add.
...
- inversion ID. subst. destruct H3.
  + inversion RED; subst; apply (IHe1 H za VALA).
  + inversion RED; subst; apply (IHe2 H zb VALB).
Qed.
...
Lemma undefined_variable (e : expr) (s : state Z) (id : id)
  ...
  forall (z : Z), ~ ([| e |] s => z).
Proof. intros z H. destruct (defined_expression e s z id H ID). apply (UNDEF x). assumption. Qed.

(* The evaluation relation is deterministic *)
Lemma eval_deterministic (e : expr) (s : state Z) (z1 z2 : Z)
      (E1 : [| e |] s => z1) (E2 : [| e |] s => z2) :
  z1 = z2.
Proof. generalize dependent z2. induction E1; intros; inversion E2; subst;
try reflexivity;
try apply (state_deterministic Z s i z z2 VAR VAR0);
try (apply (IHE1_1 za0) in VALA; apply (IHE1_2 zb0) in VALB; subst; congruence).
Qed.
...
- inversion EV. subst. apply bs_Var. apply FV.
  + apply v_Var.
  + assumption.
...
Proof. intros n s. destruct (EQ n s). split; assumption. Qed.

Lemma eq_trans (e1 e2 e3 : expr) (EQ1 : e1 ~~ e2) (EQ2 : e2 ~~ e3):
  e1 ~~ e3.
Proof. intros n s. destruct (EQ1 n s). destruct (EQ2 n s). split.
- intros. apply H1. apply H. assumption.
- intros. apply H0. apply H2. assumption.
Qed.
...
Lemma eq_eq_ceq (e1 e2 : expr) :
  e1 ~~ e2 <-> e1 ~c~ e2.
Proof. split.
- intro H. intro C. induction C.
  + assumption.
  + intros n s. split;
    intro; inversion H0; subst; apply IHC in VALA; econstructor; eassumption.
  + intros n s. split;
    intro; inversion H0; subst; apply IHC in VALB; econstructor; eassumption.
- intro H. apply (H Hole).
Qed.
...
Module SmallStep.
  ...
  Proof. induction HE.
  - apply reach_base.
  - apply (reach_step s e e'); assumption.
  Qed.

  Lemma ss_reachable_eval s e z (HR: s |- e ~~> (Nat z)) : s |- e -->> (Nat z).
  Proof. remember (Nat z). induction HR; subst.
  - apply se_Stop.
  - apply (se_Step s e e').
    + assumption.
  ...
  Proof. induction H1.
  - inversion H2.
  - apply IHss_eval in H2. apply (se_Step s e e'); assumption.
  Qed.
  ...
  Proof. induction H1.
  - assumption.
  - apply IHss_reachable in H2. apply (reach_step s e e'); assumption.
  Qed.
  ...
  Proof. intros s H. inversion HV. subst. inversion H. inversion H0. Qed.

  Lemma normal_form_is_not_a_value : ~ forall (e : expr), normal_form e -> is_value e.
  Proof. intro H. pose (Nat 1 [%] Nat 0). pose (H e). assert (normal_form e).
  - intros s H0. inversion H0. inversion H1; subst.
    + inversion LEFT.
    + inversion RIGHT.
    + inversion EVAL. subst. inversion VALB. subst. apply NZERO. reflexivity.
  - apply H in H0. inversion H0.
  Qed.

  Lemma ss_nondeterministic : ~ forall (e e' e'' : expr) (s : state Z), s |- e --> e' -> s |- e --> e'' -> e' = e''.
  Proof. intros H.
    pose ((Nat 1 [+] Nat 2) [*] (Nat 3 [+] Nat 4)).
    pose ((Nat (1+2)) [*] (Nat 3 [+] Nat 4)).
    pose ((Nat 1 [+] Nat 2) [*] (Nat (3+4))).
    assert (H1 : [] |- e --> e0).
    - apply ss_Left. apply ss_Bop. apply bs_Add; apply bs_Nat.
    - assert (H2 : [] |- e --> e1).
      + apply ss_Right. apply ss_Bop. apply bs_Add; apply bs_Nat.
      + assert (H3 : (e0 = e1) -> False).
        * intro. inversion H0.
        * apply H3. apply (H e e0 e1 []); assumption.
  Qed.
  ...
  Lemma ss_deterministic_step (e e' : expr)
                         ...
                         (H1   : s |- e --> (Nat z))
                         (H2   : s |- e --> e') : e' = Nat z.
  Proof. inversion H1; subst.
  - inversion H2. subst. apply (state_deterministic Z s i z0 z VAL0) in VAL. subst. reflexivity.
  - inversion H2; subst.
    + inversion LEFT.
    + inversion RIGHT.
    + apply (eval_deterministic (Bop op (Nat zl) (Nat zr)) s z0 z EVAL0) in EVAL. subst. reflexivity.
  Qed.
  ...
  Proof. induction HR.
  - apply reach_base.
  - apply (reach_step s (C <~ e) (C <~ e')).
    + clear IHHR HR e''. induction C; simpl.
      * assumption.
  ...
  - induction HR2.
    + apply reach_base.
    + apply (reach_step s (Bop op e e0) (Bop op e e') (Bop op e e'')).
      * apply ss_Right. assumption.
      * assumption.
  - apply (reach_step s (Bop op e e2) (Bop op e' e2) (Bop op e'' e2')).
      * apply ss_Left. assumption.
      * assumption.
  Qed.
  ...
  Proof. apply (reach_step s (Bop op (Nat za) (Nat zb)) (Nat z) (Nat z)).
  - apply ss_Bop. inversion H; subst;
    try apply (eval_deterministic e1 s za za0 VALA) in VALA0;
    try apply (eval_deterministic e2 s zb zb0 VALB) in VALB0;
    subst; eauto.
  - apply reach_base.
  Qed.
  ...
  Proof. apply ss_reachable_eval. apply (ss_reachable_trans s (Bop op e1 e2) (Bop op (Nat za) (Nat zb))).
  - apply ss_subst_binop.
    + apply ss_eval_reachable. apply IHe1.
    + apply ss_eval_reachable. apply IHe2.
  - apply (ss_bop_reachable s e1 e2); assumption.
  Qed.

  #[export] Hint Resolve ss_eval_binop : core.

  Lemma ss_step_eval s e e' z
        (HStep : s |- e --> e')
        (HEval : [| e' |] s => z) :
        [| e |] s => z.
  Proof. generalize dependent z. induction HStep; intros;
    inversion HEval; subst; try econstructor; eauto.
  Qed.
  ...
  Proof. split; intro.
  - induction H; eauto.
  - remember (Nat z). induction H.
    + inversion Heqe0. subst. apply bs_Nat.
    + subst. apply (ss_step_eval s e e' z).
      * assumption.
...
Module StaticSemantics.
  ...
  Lemma type_preservation_false : ~ (forall e t t' (HS: t' << t) (HT: e :-: t), forall st e' (HR: st |- e ~~> e'), e' :-: t').
  Proof. intro H. pose (Nat 2 [+] Nat 3). assert (e :-: Int).
  - apply type_Add; apply type_N; intro; inversion H0; discriminate.
  - pose (H e Int Bool subt_base H0 [] e (reach_base _ _)). inversion t.
  Qed.

  Lemma type_bool e (HT : e :-: Bool) :
    forall st z (HVal: [| e |] st => z), zbool z.
  Proof. remember Bool. intros. induction HT; inversion HVal; subst;
  try congruence;
  try (right; reflexivity);
  try (left; reflexivity);
  try (destruct BOOLA; destruct BOOLB; subst; auto).
  Qed.
...
Module Renaming.
  ...
  Proof. destruct r. destruct b. destruct a.
    exists (exist (fun f => Bijective f) x0 (ex_intro _ x (conj e0 e))).
    intro y. apply e.
  Qed.
  ...
  Proof. induction st.
  - reflexivity.
  - destruct r. destruct r'. destruct a. simpl. rewrite IHst. rewrite Hinv. reflexivity.
  Qed.

  Lemma bijective_injective (f : id -> id) (BH : Bijective f) : Injective f.
  Proof. intros x y. intro H. destruct BH. destruct H0. congruence. Qed.

  Lemma eval_renaming_binds (x: id) (st: state Z) (z: Z) (r: renaming):
    st / x => z -> (rename_state r st) / (rename_id r x) => z.
  Proof. intro. induction H; destruct r; simpl.
  - apply st_binds_hd.
  - apply st_binds_tl.
    + intro. apply (bijective_injective x0 b) in H1. contradiction.
    + apply IHst_binds.
  Qed.

  Lemma eval_renaming_inv_fwd (e : expr) (st : state Z) (z : Z) (r : renaming)
        (H : [| e |] st => z) : [| rename_expr r e |] (rename_state r st) => z.
  Proof. induction H; simpl; eauto.
    apply bs_Var. apply eval_renaming_binds. assumption.
  Qed.

  Lemma eval_renaming_invariance (e : expr) (st : state Z) (z : Z) (r: renaming) :
    [| e |] st => z <-> [| rename_expr r e |] (rename_state r st) => z.
  Proof. split; intro H.
  - apply eval_renaming_inv_fwd. assumption.
  - destruct (renaming_inv r).
    apply (eval_renaming_inv_fwd (rename_expr r e) (rename_state r st) z x) in H.
    rewrite re_rename_expr in H.
    + rewrite re_rename_state in H; assumption.
    + assumption.
  Qed.
